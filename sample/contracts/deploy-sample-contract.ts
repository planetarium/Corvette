...
function normalizeUrlOrPath(url: string): string {
  try {
...
async function main() {
  ...
  let chain;
  const chainDefUrl = normalizeUrlOrPath(Deno.args[0]);

  try {
    chain = (await import(chainDefUrl, { assert: { type: "json" } }))
      .default as Chain;
  } catch (e) {
    if (!(e instanceof TypeError)) throw e;
    chain = (await import(chainDefUrl)).default as Chain;
  }

  const keystore = (await import(normalizeUrlOrPath(Deno.args[1]), {
    assert: { type: "json" },
  }))
    .default;
